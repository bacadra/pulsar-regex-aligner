...
class Dialog {
  ...
  tabularize(separator_regex, editor) {
    ...
    editor.mutateSelectedText((selection) => {
      ...
      const result = _.chain(padded_lines).zip(matches).map((e) => {
        let line = _(e).first()
        matches = _(e).last()
        line = _.chain(line).zip(matches).flatten().compact().value().join(' ')
        return this.stripTrailingWhitespace(line);}).value().join("\n")
  ...
  paddingColumn(col_index, matrix) {
    ...
    let cell_size = 0;
    const column = _(matrix).map((line) => {
  ...
  stripSpaces(lines) {
    ...
    return _.map(lines, cells => cells = _.map(cells, (cell, i) => {
      if (i === 0) {
